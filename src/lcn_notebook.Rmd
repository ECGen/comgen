...
```{r load-data, echo = FALSE, eval = TRUE, results = "hide", warnings = FALSE, message = FALSE}
...
## Lecanoras merged
                                        # Loading data
xgal.size <- read.csv("../data/ONC_Xgal_SizeData_May2011.csv")
...
rflp.d <- readLines('../data/rflp_queller_goodnight.txt')
...
if (!(all(c(all(rownames(as.matrix(rflp.d)) == 
    ...
    print("Distance matrices good to go!")
                                        ...
                                        # Bipartite analysis
nperm <- 20
if (!(file.exists("../data/nest_rel_onc.rda"))){
    nest.onc <- nestedness(onc.com.rel[, colnames(onc.com.rel) != "ds"], 
                           n.nulls = 999)
    dput(nest.onc, "../data/nest_rel_onc.rda")
}else{
  nest.onc <- dget("../data/nest_rel_onc.rda")
}
if (!(file.exists("../data/null_mod_onc.csv"))){
    ...
    dput(mods.onc, "../data/mod_list_onc.rda")
    write.csv(slot(obs.mod.onc, "likelihood"), 
              "../data/obs_mod_onc.csv", 
              row.names = FALSE)
    write.csv(nul.mod.onc, 
              "../data/null_mod_onc.csv", 
              row.names = FALSE)
}else{
    obs.mod.onc <- read.csv("../data/obs_mod_onc.csv")[1]
    nul.mod.onc <- read.csv("../data/null_mod_onc.csv")[,1]
    z.mod.onc <- (obs.mod.onc - mean(nul.mod.onc)) / sd(nul.mod.onc)
    mods.onc <- dget("../data/mod_list_onc.rda")
}
...
if (!file.exists("../data/onc_nmds.csv")){
    nms.info.onc <- capture.output(nms.onc <- nmds.min(nmds(
        vegdist(onc.com.rel), 2, 2)))
    write.csv(nms.onc, "../data/onc_nmds.csv", 
              row.names = FALSE)
    write.table(nms.info.onc, 
                "../data/onc_nmds_info.txt", 
                col.names = FALSE, row.names = FALSE)
}else{nms.onc <- read.csv("../data/onc_nmds.csv")}
                                        # Network ordination
if (!(file.exists("../data/conet_nmds.csv"))){
    cn.nmds.stats.onc <- capture.output(
      cn.nms.onc <- nmds.min(nmds(cn.d.onc, 2, 2)))
    write.csv(cn.nms.onc, "../data/conet_nmds.csv", 
              row.names = FALSE)
    write.table(cn.nmds.stats.onc, 
                "../data/conet_nmds_info.txt", 
                col.names = FALSE, row.names = FALSE)
}else{cn.nms.onc <- read.csv("../data/conet_nmds.csv")}
                                        # Vector fitting
...
tree.id <- do.call(rbind, strsplit(names(ptc.onc), split = " "))[, 1]
                                        # add chemistry data
onc.nc <- read.csv("../data/ONC_phytochem_NC.csv")
onc.tan <- read.csv("../data/ONC_phytochem_tannin.csv")
...
onc.nc$rCN <- onc.nc$N / onc.nc$C
                                        # pH data
onc.ph <- read.csv("../data/ONC_Bark_lichen_pH_data.csv")
...
col.pal <- RColorBrewer::brewer.pal((max(unlist(mods.onc))), "Paired")
                                        # Figure ordinations
                                        # Communities
if (file.exists("../data/nms_com_onc.rda")){
    nms.com <- dget(file = "../data/nms_com_onc.rda")    
}else{
    set.seed(12345)
    nms.com <- nmds(vegdist(onc.com.rel), 2, 3)
    dput(nms.com, file = "../data/nms_com_onc.rda")
}
                                        # Networks
if (file.exists("../data/nms_cn_onc.rda")){
    nms.cn <- dget(file = "../data/nms_cn_onc.rda")    
}else{
    set.seed(12345)
    nms.cn <- nmds(cn.d.onc, 1, 2)
    dput(nms.cn, file = "../data/nms_cn_onc.rda")
}
...
vec.cn <- envfit(ord.cn, env = vec.env, perm = 10000, 
                  choices = c(1,2))
                                        # onc
if (!("mod_obsval_onc.csv" %in% dir("../data"))){
  ...
  mod.onc <- read.csv(file = "../data/mod_obsval_onc.csv")[, 1]
}
if (!("mod_simvals_onc.csv" %in% dir("../data"))){
        ...
        write.csv(mods.onc.sweb, 
                  file = "../data/mod_simvals_onc.csv", 
                  row.names = FALSE)
# nest.onc <- bipartite::nestedness(onc.com.rel)
}else{
    mods.onc.sweb <- read.csv(
      file = "../data/mod_simvals_onc.csv")[, 1]
}

                                        # pit
if (!("mod_obsval_pit.csv" %in% dir("../data"))){
        mod.pit <- slot(bipartite::computeModules(rel(pit.com), deep = FALSE), 
                         "likelihood")
        write.csv(mod.pit, 
                  file = "../data/mod_obsval_pit.csv", 
                  row.names = FALSE)
}else{
    mod.pit <- read.csv(
      file = "../data/mod_obsval_pit.csv")[, 1]
}
if (!("mod_simvals_pit.csv" %in% dir("../data"))){
...
for (i in 1:dim(pit.sweb)[3]){pit.sweb[,, i] <- rel(pit.sweb[,, i])}
  ...
  write.csv(mods.pit.sweb, 
            file = "../data/mod_simvals_pit.csv", 
            row.names = FALSE)
# nest.pit <- bipartite::nestedness(pit.com.rel)
}else{
  mods.pit.sweb <- read.csv(
    file = "../data/mod_simvals_pit.csv")[, 1]
}

### Wild data
###
###
x <- read.csv('../data/lco_Apr2012.csv')
                                        #remove notes
...
wild.q <- lapply(split(x,x$tree),function(x) x[,-1:-4])
                                        #data from lamit
env <- read.csv('../data/Uinta2012_all_data_from_Lamit.csv')
...
prb.wild <- tapply(env$Pct.Roughness,env.split,mean) #percent rough bark
                                        #age
age <- read.csv("../data/UintaMaster_LichenHeritNL_FallSpring_2012_ForLau.csv")
dbh <- age$DBH.cm_01
...
## pH ~ genotype
ph.reml <- lme4::lmer(I(log(pH)) ~ (1 | geno), 
...
coord <- gplot(abs(meanNet(cn.mu.onc)), gmode = "digraph", 
      displaylabels = TRUE, 
      edge.lwd = abs(meanNet(cn.mu.onc)) * 20, 
      edge.col = net.col,
      vertex.col = "black", 
      vertex.cex = 0.5,
      arrowhead.cex = 0.5, 
