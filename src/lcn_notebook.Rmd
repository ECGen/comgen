...
pkg.list <- c("MuMIn", "lme4", "RLRsim", "vegan", "ecodist", "bipartite", "RColorBrewer", "enaR", "devtools")
...
if (!("coNet" %in% installed.packages()[, 1])){
    devtools::install_github("CommunityGeneticsAnalyses/coNet")
                                        ...
                                        # Load libraries
sapply(c(pkg.list, "ComGenR", "coNet"), library, quietly = TRUE, character.only = TRUE)
...
h2.tab[5, "Response"] <- "Lichen Network"
                                        ...
                                        # Adding chemistry
vegan::adonis2(onc.com.rel[!is.na(onc.dat[, "CN"]),]^(1/1) ~  BR + PC + SR + CT + CN, data = na.omit(onc.dat), perm = 10000, mrank = TRUE)
cn.d.onc.chem <- as.dist(as.matrix(cn.d.onc)[!is.na(onc.dat[, "CN"]),
                                     !is.na(onc.dat[, "CN"])])
vegan::adonis2(cn.d.onc.chem ~ geno + BR +  PC + SR + CT + CN, 
               data = na.omit(onc.dat), permutations = 10000, mrank = TRUE)
...
colnames(onc.tan)[grep("X.CT", colnames(onc.tan))] <- "CT"
                                        # add C:N ratio
onc.nc$rCN <- onc.nc$N / onc.nc$C
                                        # remove trees not in onc.dat
                                        # makes missing samples NA
onc.dat <- data.frame(onc.dat, 
                   C = onc.nc[match(onc.dat[, "tree.id"], 
                        onc.nc[, "tree.id"]), "C"],
                   N = onc.nc[match(onc.dat[, "tree.id"], 
