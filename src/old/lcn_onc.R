...
## SES ~ Roughness
summary(MASS::rlm(sqrt(abs(os))~prb.onc))
summary(lm(sqrt(abs(os))~prb.onc))

###transform percent rough bark too
cor.test(sqrt(abs(os)), sqrt(prb.onc))
...
prb.d <- dist(as.matrix(prb.onc))
mantel(os.d~prb.d,  mrank  =  FALSE,  nperm  =  10000)
...
### PRB~genotype
oneway.test(prb~og)
prb.onc.mu <- tapply(prb.onc,  og, mean)
prb.onc.se <- tapply(prb.onc,  og, function(x) sd(x)/length(x))
barplot2(prb.onc.mu[order(prb.onc.mu,  decreasing = TRUE)],  
         plot.ci = TRUE, 
         ci.u  =  prb.onc.mu[order(prb.onc.mu,  decreasing = TRUE)] + 
             prb.onc.se[order(prb.onc.mu,  decreasing = TRUE)], 
         ci.l  =  prb.onc.mu[order(prb.onc.mu,  decreasing = TRUE)] - 
             prb.onc.se[order(prb.onc.mu,  decreasing = TRUE)], 
         ylab = 'Percent Rough Bark')
                                        #beside plot
mu <- rbind('Percent Rough Bark' = prb.onc.mu, 'SES' = osgmu[match(names(prb.onc.mu), names(osgmu))])
se <- rbind('prb.onc.se' = prb.onc.se, 'ses.se' = osgse[match(names(prb.onc.se), names(osgse))])
...
ch.plot(ch.x, og)
abline(lm(osgmu ~ prb.onc.mu), lty = 2)
...
## Araujo network
cn <- lapply(oq, coNets, ci.p = 80) 
cn.d <- netDist(cn)
adonis(cn.d ~ prb.onc)

## genotype averaged nets
gan <- list()
for (i in 1:length(unique(og))){
    n <- cn[og == unique(og)[i]]
    gan[[i]] <- Reduce("+", n) / length(n)
}
names(gan) <- unique(og)

par(mfrow = c(4,4))
for (i in 1:length(gan)){gplot(gan[[i]])}
