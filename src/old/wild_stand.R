...
wenv <- read.csv('../data/lichen_networks/uintah_trees/uintah_2012_lco_trees.csv')
...
names(ses.t) <- names(ses.ocs)
                                        #centrality
cen.t <- unlist(lapply(dn.t,function(x) sna::centralization(x,FUN='degree')))
                                        #degree
