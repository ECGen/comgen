...
obs.cs <- unlist(lapply(onc.q,cscore))
                                        #load ses values
onc.ses <- dget('../data/lichen_networks/onc_tree_ses.Rdata')
onc.p <- dget('../data/lichen_networks/onc_tree_pval.Rdata')
...
gen.d <- read.csv(file='../data/lichen_networks/ONC_MSAT_datafromnash.csv')[,-1]
gen.d[is.na(gen.d)] <- 0
gen.d <- as.dist((gen.d))
                                        #community data
...
###Co-occurrences
                                        #co-occurrence
stand.null <- unlist(dget(file='../data/lichen_networks/onc_stand_null.Rdata'))
...
c(stand.ses,stand.ses.p)
                                        #
onc.treemean.ses <- dget(file='../results/onc_ses_treemean.Rdata')
onc.treemean.ses
                                        #
onc.ses <- dget(file='../data/lichen_networks/onc_tree_ses.Rdata')
onc.ses[is.na(onc.ses)] <- 0
...
onc.nest <- read.csv(file='../results/onc_nest_tree.csv')
onc.geno.nest <- read.csv(file='../results/onc_nest_geno.csv')
