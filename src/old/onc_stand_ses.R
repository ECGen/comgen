...
onc <- garden.data[g1 == 'onc',]
					#stand
if (!file.exists("../data/lichen_networks/onc_stand_null.Rdata")){
    stand.null <- nullCom(onc[ ,7:15])
    stand.null <- lapply(stand.null, cscore)
    dput(stand.null, file="../data/lichen_networks/onc_stand_null.Rdata")
}else{
    stand.null <- dget("../data/lichen_networks/onc_stand_null.Rdata")
}
