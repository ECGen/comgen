...
### Re-run analyses accounting for paired data

## 1. Species accumulation curves
## 2. Community analyses using the differences (A, R, H, Comp)
## 3. Indicator species of resistance
## 4. SEM to test causal pathways
## 5. Plots of A, R, H, ordination, and SEM
...
## Data are in ~/storage/scrl
lichen=list()
for (i in 1:length(dir())){
    lichen[[i]] <- read.csv(dir("../data/scrl", full = TRUE)[i])
}
names(lichen) <- dir()
names(lichen)
summary(lichen[[3]])
